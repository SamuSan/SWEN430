...
public class Parser {
    ...
    private List<Stmt> parseCaseBlock() {
    ...
    private Stmt parseSwitch() {
        ...
        Expr.Variable switchVariable = parseSwitchVariable();
        ...
        while (index < tokens.size()
                && !(tokens.get(index) instanceof RightCurly)) {
            Expr.Binary caseCondition = parseCaseCondition(switchVariable);
            ArrayList<Stmt> statements = (ArrayList<Stmt>) parseCaseBlock();
            cases.put(caseCondition, statements);
        }
        
        match("}");
        return new Stmt.Switch(cases);
    }

    private Expr.Variable parseSwitchVariable() {
        checkNotEof();
        Expr.Variable switchVariable = new Expr.Variable(
                tokens.get(index).text, sourceAttr(index, index));
        index = index + 1;
    ...
    private Expr parseConditionExpression() {
        ...
        if (index < tokens.size() && tokens.get(index) instanceof LessEquals) {
            ...
            return new Expr.Binary(Expr.BOp.IS, lhs, rhs, sourceAttr(start,
                    index - 1));
        }
